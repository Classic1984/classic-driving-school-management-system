<x-app-layout>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <div class="print-hidden flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <form method="post" action="{{ route('corporate-payments.receipt.whatsapp', $payment) }}">
                        @csrf
            ...
            @if (session('status') === 'receipt-emailed')
    ...
    <x-modal name="send-email" focusable>
        <form method="post" action="{{ route('corporate-payments.receipt.email', $payment) }}" class="p-6 space-y-4">
            @csrf
            <h2 class="text-lg font-bold text-gray-900">{{ __('Send Receipt by Email') }}</h2>
            <p class="text-sm text-gray-500">{{ __('The receipt PDF will be attached automatically.') }}</p>
            <div>
                <x-input-label for="recipient_email" :value="__('Recipient Email')" />
                <x-text-input id="recipient_email" name="recipient_email" type="email" class="block w-full mt-1" :value="old('recipient_email', $payment->invoice->company->email)" required autofocus />
