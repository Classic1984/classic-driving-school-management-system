<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @elseif (session('status') === 'driver-added')
                <p class="text-sm font-medium text-green-600">{{ __('Driver added successfully.') }}</p>
            @elseif (session('status') === 'driver-removed')
                <p class="text-sm font-medium text-green-600">{{ __('Driver removed successfully.') }}</p>
            @endif
            ...
            <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl overflow-hidden">
                ...
                <div id="drivers" class="p-6 sm:p-8 border-t border-gray-100">
                    <h4 class="flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-gray-500">
                    ...
                    <form method="post" action="{{ route('corporate-companies.drivers.store', $company) }}" class="mt-3 flex flex-wrap items-end gap-3">
                        ...
                        <div class="flex-1 min-w-[10rem]">
                            <x-input-label for="driver_name" :value="__('Driver Name')" class="text-xs" />
                            <x-text-input id="driver_name" name="name" type="text" class="block w-full mt-1 text-sm" :value="old('name')" required />
                        </div>
                        <div class="flex-1 min-w-[8rem]">
                            <x-input-label for="driver_phone" :value="__('Phone (optional)')" class="text-xs" />
                            <x-text-input id="driver_phone" name="phone" type="text" class="block w-full mt-1 text-sm" :value="old('phone')" />
                        </div>
                        <div class="flex-1 min-w-[8rem]">
                            <x-input-label for="driver_license_number" :value="__('License No. (optional)')" class="text-xs" />
                            <x-text-input id="driver_license_number" name="license_number" type="text" class="block w-full mt-1 text-sm" :value="old('license_number')" />
                        </div>
                        <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-black hover:bg-gray-900 px-4 py-2 text-sm font-bold text-amber-400 transition">
                            {{ __('Add Driver') }}
                        </button>
                    </form>
                    <x-input-error class="mt-2" :messages="$errors->get('name')" />

                    @if ($company->drivers->isEmpty())
                        <p class="mt-4 text-sm text-gray-500">{{ __('No drivers enrolled yet.') }}</p>
                    @else
                        <ul class="mt-4 divide-y divide-gray-100 ring-1 ring-gray-200 rounded-lg overflow-hidden">
