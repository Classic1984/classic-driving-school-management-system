                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div class="flex items-start gap-2 rounded-lg bg-gray-50 p-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-bold text-gray-900">{{ $student->date_of_birth->format('l, M j, Y') }}</p>
                                    </div>
                                </div>
                                <div class="flex items-start gap-2 rounded-lg bg-gray-50 p-3">
                                    ...
                                    @foreach ($student->courses as $enrolledCourse)
                                        ...
                                        <div class="rounded-xl ring-1 ring-gray-200 p-4">
                                            ...
                                            <div class="mt-1 text-xs text-gray-500">
                                                {{ __('Start Date') }}: {{ optional($enrolledCourse->pivot->enrolled_at)->format('l, M j, Y') ?? '—' }}
                                                &middot;
                                                {{ __('Expected Completion') }}: {{ optional($enrolledCourse->pivot->expectedCompletionDate())->format('l, M j, Y') ?? '—' }}
                                            </div>
                                            @php
                                                ...
                                                <div x-show="!editingAssessment" class="mt-1">
                                                    @if ($assessment)
                                                        ...
                                                        <p class="mt-1 text-xs text-gray-400">
                                                            {{ __('Assessed by') }} {{ $assessment->assessedBy?->name ?? '—' }} &middot; {{ $assessment->assessed_at->format('l, M j, Y') }}
                                                        </p>
                                                    @else
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="divide-y divide-gray-100 bg-white">
                                        @forelse ($student->attendances as $attendance)
                                            <tr>
                                                <td class="px-3 py-3 text-sm text-gray-600">{{ $attendance->date->format('l, M j, Y') }}</td>
                                                        ...
                                                        @if ($studentService->processingProgressPercent() !== null)
                                                            <div class="mt-1 text-xs text-gray-500">
                                                                ...
                                                                @if ($studentService->expectedReadyAt())
                                                                    &middot; {{ __('Ready by :date', ['date' => $studentService->expectedReadyAt()->format('l, M j, Y')]) }}
                                                                @endif
                                    ...
                                    <tbody class="divide-y divide-gray-100 bg-white">
                                        @forelse ($student->certificates as $certificate)
                                            <tr>
                                                ...
                                                <td class="px-3 py-3 text-sm text-gray-600">{{ $certificate->issue_date->format('l, M j, Y') }}</td>
                                                <td class="px-3 py-3 text-sm">
