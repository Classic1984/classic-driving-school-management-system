        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl overflow-hidden">
                <div class="relative overflow-hidden bg-black p-6 sm:p-8">
                    <svg class="pointer-events-none absolute -right-8 -top-8 h-48 w-48 text-amber-500/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="0.75"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $trendingUpIconPath }}" /></svg>
                    ...
                    <div class="relative flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-amber-500/15 text-amber-400 ring-1 ring-amber-400/30">
                                <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $trendingUpIconPath }}" /></svg>
                            </span>
                            <div>
                                <h3 class="text-xl font-bold text-white">{{ __('Lead Conversion Report') }}</h3>
                                <p class="text-sm text-gray-400">{{ __('How inquiries move from lead to enrolled student') }} — {{ __($label) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 print:hidden">
                            ...
                            <a href="{{ route('lead-conversion-report.export', ['period' => $period]) }}" class="inline-flex items-center gap-2 rounded-lg ring-1 ring-green-400/30 bg-green-500/10 px-4 py-2 text-sm font-semibold text-green-400 hover:bg-green-500/20 transition">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                                {{ __('Export Excel') }}
                            </a>
                            <a href="{{ route('lead-conversion-report.export-pdf', ['period' => $period]) }}" class="inline-flex items-center gap-2 rounded-lg ring-1 ring-red-400/30 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-400 hover:bg-red-500/20 transition">
                    ...
                    <div class="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-6">
                        <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-amber-500/15 via-gray-900 to-gray-900 ring-1 ring-amber-400/30 p-4">
                            <svg class="pointer-events-none absolute -right-3 -bottom-3 h-16 w-16 text-amber-400/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $envelopeIconPath }}" /></svg>
                        ...
                        <div class="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
                            ...
                            <p class="text-xs uppercase tracking-wider text-blue-400 mt-2">{{ __('New') }}</p>
                            <p class="text-xl font-bold text-white mt-0.5">{{ $summary['new'] }}</p>
                        </div>
                        <div class="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
                            ...
                            <p class="text-xs uppercase tracking-wider text-amber-400 mt-2">{{ __('Contacted') }}</p>
                            <p class="text-xl font-bold text-white mt-0.5">{{ $summary['contacted'] }}</p>
                        </div>
                        <div class="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
                            <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-green-500/15 text-green-400">
                            ...
                            <p class="text-xs uppercase tracking-wider text-green-400 mt-2">{{ __('Converted') }}</p>
                            <p class="text-xl font-bold text-white mt-0.5">{{ $summary['converted'] }}</p>
                        </div>
                        <div class="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
                            <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500/15 text-red-400">
                            ...
                            <p class="text-xs uppercase tracking-wider text-red-400 mt-2">{{ __('Lost') }}</p>
                            <p class="text-xl font-bold text-white mt-0.5">{{ $summary['lost'] }}</p>
                        </div>
