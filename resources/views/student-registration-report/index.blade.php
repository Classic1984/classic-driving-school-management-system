        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl overflow-hidden">
                <div class="relative overflow-hidden bg-black p-6 sm:p-8">
                    <svg class="pointer-events-none absolute -right-8 -top-8 h-48 w-48 text-amber-500/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="0.75"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $userPlusIconPath }}" /></svg>
                    ...
                    <div class="relative flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-amber-500/15 text-amber-400 ring-1 ring-amber-400/30">
                                <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $userPlusIconPath }}" /></svg>
                            </span>
                            <div>
                                <h3 class="text-xl font-bold text-white">{{ __('Student Registration Report') }}</h3>
                                <p class="text-sm text-gray-400">{{ __('New student sign-ups over time') }} — {{ __($label) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 print:hidden">
                            ...
                            <a href="{{ route('student-registration-report.export', ['period' => $period]) }}" class="inline-flex items-center gap-2 rounded-lg ring-1 ring-green-400/30 bg-green-500/10 px-4 py-2 text-sm font-semibold text-green-400 hover:bg-green-500/20 transition">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                                {{ __('Export Excel') }}
                            </a>
                            <a href="{{ route('student-registration-report.export-pdf', ['period' => $period]) }}" class="inline-flex items-center gap-2 rounded-lg ring-1 ring-red-400/30 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-400 hover:bg-red-500/20 transition">
                    ...
                    <div class="relative grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500/15 via-gray-900 to-gray-900 ring-1 ring-amber-400/30 p-5">
                            <svg class="pointer-events-none absolute -right-4 -bottom-4 h-24 w-24 text-amber-400/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="0.75"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $userPlusIconPath }}" /></svg>
                            <div class="relative flex items-center gap-3">
                                ...
                                <p class="text-xs font-semibold uppercase tracking-wider text-gray-300">{{ __('New Students') }}</p>
                            </div>
                            <p class="relative text-4xl font-extrabold text-amber-400 mt-3">{{ $students->count() }}</p>
                        </div>
                        ...
                        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500/15 via-gray-900 to-gray-900 ring-1 ring-emerald-400/30 p-5">
                            <svg class="pointer-events-none absolute -right-4 -bottom-4 h-24 w-24 text-emerald-400/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="0.75"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $shieldCheckIconPath }}" /></svg>
                            <div class="relative flex items-center gap-3">
                                ...
                                <p class="text-xs font-semibold uppercase tracking-wider text-gray-300">{{ __('Still Active') }}</p>
                            </div>
                            <p class="relative text-4xl font-extrabold text-emerald-400 mt-3">{{ $activeStudentsCount }}</p>
                        </div>
                    </div>
                </div>
                                    ...
                                    @forelse ($students as $student)
                                        ...
                                        <tr class="hover:bg-amber-50/40 transition">
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                <a href="{{ route('students.show', $student) }}" class="group flex items-center gap-2.5">
                                                    <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-amber-100 text-xs font-bold text-amber-800">{{ $registrationInitials }}</span>
                                                    <span class="font-semibold text-gray-900 group-hover:text-amber-600 print:text-gray-900">{{ $student->name }}</span>
                                                </a>
                                            </td>
