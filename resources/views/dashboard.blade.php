    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($learnersPermitRequests->isNotEmpty())
                <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl p-8 mt-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-xl font-bold text-gray-800">{{ __("Learner's Permit Requests") }}</h3>
                        <a href="{{ route('learners-permit-report.index') }}" class="text-sm text-amber-600 hover:underline">{{ __('View Full Report') }}</a>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach ($learnersPermitRequests as $studentService)
                                    <tr>
                                        <td class="py-2 pr-4">
                                            <a href="{{ route('students.show', $studentService->student_id) }}" class="text-amber-600 hover:underline">
                                                {{ $studentService->student->name }}
                                        ...
                                        <td class="py-2">
                                            <form method="post" action="{{ route('student-services.processing-status.update', $studentService) }}">
                                                @csrf
                                                @method('patch')
                                                <input type="hidden" name="processing_status" value="completed">
                                                <button type="submit" class="text-sm text-amber-600 hover:underline">{{ __('Mark Obtained') }}</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
