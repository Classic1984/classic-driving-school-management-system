<x-guest-layout>
    ...
    <form method="post" action="{{ route('instructor.verify-otp.store') }}">
        @csrf
    ...
    <p class="mt-4 text-center text-xs text-gray-400">
        <a href="{{ route('instructor.login') }}" class="text-amber-600 hover:underline">{{ __('Use a different phone number') }}</a>
    </p>
</x-guest-layout>
