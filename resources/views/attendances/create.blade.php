    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="flex items-center gap-4 mb-6 px-4 sm:px-0">
                ...
                <div>
                    <h3 class="text-2xl font-extrabold text-gray-900">{{ __('Log Student Training') }}</h3>
                    <p class="text-sm text-gray-500">{{ __('Record a training session for a student') }}</p>
                </div>
            </div>
            ...
            <div class="p-4 sm:p-8 bg-white shadow-sm ring-1 ring-gray-200 sm:rounded-xl">
                <form method="post" action="{{ route('attendances.store') }}" class="space-y-6">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('attendances.index') }}" class="text-sm text-gray-600 hover:underline">{{ __('Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
