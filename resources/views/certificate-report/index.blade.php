        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl overflow-hidden">
                <div class="relative overflow-hidden bg-black p-6 sm:p-8">
                    <svg class="pointer-events-none absolute -right-8 -top-8 h-48 w-48 text-amber-500/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="0.75"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $certificateIconPath }}" /></svg>
                    ...
                    <div class="relative flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-amber-500/15 text-amber-400 ring-1 ring-amber-400/30">
                                <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $certificateIconPath }}" /></svg>
                            </span>
                            <div>
                                <h3 class="text-xl font-bold text-white">{{ __('Certificate Report') }}</h3>
                                <p class="text-sm text-gray-400">{{ __('Certificates issued to students') }} — {{ __($label) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 print:hidden">
                            ...
                            <a href="{{ route('certificate-report.export', ['period' => $period]) }}" class="inline-flex items-center gap-2 rounded-lg ring-1 ring-green-400/30 bg-green-500/10 px-4 py-2 text-sm font-semibold text-green-400 hover:bg-green-500/20 transition">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                                {{ __('Export Excel') }}
                            </a>
                            <a href="{{ route('certificate-report.export-pdf', ['period' => $period]) }}" class="inline-flex items-center gap-2 rounded-lg ring-1 ring-red-400/30 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-400 hover:bg-red-500/20 transition">
                    ...
                    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500/15 via-gray-900 to-gray-900 ring-1 ring-amber-400/30 p-5 mt-6 max-w-sm">
                        <svg class="pointer-events-none absolute -right-4 -bottom-4 h-24 w-24 text-amber-400/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="0.75"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $certificateIconPath }}" /></svg>
                        <div class="relative flex items-center gap-3">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-amber-500/15 text-amber-400">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $certificateIconPath }}" /></svg>
                            </span>
                            <p class="text-xs font-semibold uppercase tracking-wider text-gray-300">{{ __('Certificates Issued') }}</p>
                        </div>
                        <p class="relative text-4xl font-extrabold text-amber-400 mt-3">{{ $certificates->count() }}</p>
                    </div>
                </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tr class="text-left text-xs font-semibold uppercase tracking-wider text-amber-400">
                                        <th class="px-4 py-3">
                                            <span class="inline-flex items-center gap-1.5">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $certificateIconPath }}" /></svg>
                                                {{ __('Certificate #') }}
                                            </span>
                                        </th>
                                ...
                                <tbody class="divide-y divide-gray-100 bg-white">
                                    @forelse ($certificates as $certificate)
                                        @php $certificateInitials = collect(explode(' ', $certificate->student->name))->map(fn ($part) => mb_substr($part, 0, 1))->take(2)->implode(''); @endphp
                                        <tr class="hover:bg-amber-50/40 transition">
                                            <td class="px-4 py-3">
                                                <a href="{{ route('certificates.show', $certificate) }}" class="font-mono text-sm text-amber-600 hover:underline print:text-gray-900 print:no-underline">
                                                    {{ $certificate->certificate_number }}
                                                </a>
                                            </td>
                                            <td class="px-4 py-3 font-mono text-sm text-gray-600">{{ $certificate->student->student_id_number }}</td>
                                            <td class="px-4 py-3 text-sm">
                                                <a href="{{ route('students.show', $certificate->student) }}" class="group flex items-center gap-2.5">
                                                    <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-amber-100 text-xs font-bold text-amber-800">{{ $certificateInitials }}</span>
                                                    <span class="font-semibold text-gray-900 group-hover:text-amber-600 print:text-gray-900">{{ $certificate->student->name }}</span>
                                                </a>
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $certificate->issue_date->format('l, M j, Y') }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="6" class="px-4 py-10 text-center">
                                                <div class="flex flex-col items-center gap-2">
                                                    ...
                                                    <p class="text-sm text-gray-500">{{ __('No certificates issued during this period.') }}</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
