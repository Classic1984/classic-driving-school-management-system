    ...
    <div class="py-12 print:py-0">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 print:max-w-full print:px-0">
            ...
            @elseif (session('status') === 'certificate-whatsapp-failed')
                <p class="print:hidden mb-4 text-sm font-medium text-red-600">{{ __('WhatsApp could not deliver this message. Double-check the phone number is correct and on WhatsApp, or use "Open in WhatsApp" below to send it yourself.') }}</p>
            @endif
                ...
                <div class="relative border-2 border-amber-500/70 rounded-xl p-8 sm:p-14 text-center">
                    ...
                    <div class="mt-10 grid grid-cols-2 sm:grid-cols-5 gap-y-5 gap-x-2 rounded-xl bg-black/30 ring-1 ring-amber-500/20 px-4 py-5">
                        ...
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-sm">{{ $certificate->issue_date->format('l, M j, Y') }}</p>
                        </div>
                        <div class="flex flex-col items-center">
            ...
            <div class="mt-6 flex items-center gap-4 print:hidden">
                ...
                <form method="post" action="{{ route('certificates.whatsapp-link', $certificate) }}">
                    @csrf
                    <x-secondary-button type="submit" title="{{ __('Opens WhatsApp with the message ready - you press Send. No Twilio needed.') }}">
                        <svg class="h-4 w-4 -ml-1 mr-1.5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg>
                        {{ __('Open in WhatsApp') }}
                    </x-secondary-button>
                </form>
