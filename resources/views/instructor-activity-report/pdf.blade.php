<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Instructor Activity Report - {{ $label }}</title>
    <style>
        body { font-family: sans-serif; font-size: 12px; color: #111827; }
        h1 { font-size: 18px; margin-bottom: 0; }
        h2 { font-size: 11px; font-weight: normal; color: #6b7280; margin-top: 4px; }
        table { width: 100%; border-collapse: collapse; margin-top: 16px; }
        th, td { border: 1px solid #d1d5db; padding: 6px 8px; text-align: left; }
        th { background-color: #f3f4f6; text-transform: uppercase; font-size: 10px; }
    </style>
</head>
<body>
    <h1>Classic Driving School &amp; Son Nigeria Limited</h1>
    <h2>Instructor Activity Report — {{ $label }} — Generated {{ now()->format('Y-m-d H:i') }}</h2>

    <table>
        <thead>
            <tr>
                <th>Instructor</th>
                <th>Sessions Conducted</th>
                <th>Training Days Logged</th>
                <th>Students Trained</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    <td>{{ $row['instructor']->name }}</td>
                    <td>{{ $row['sessions'] }}</td>
                    <td>{{ $row['training_days'] }}</td>
                    <td>{{ $row['students_trained'] }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="4">No training was logged by an instructor during this period.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
</body>
</html>
